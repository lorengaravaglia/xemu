...
void qmp_marshal_migrate(QDict *args, QObject **ret, Error **errp)
{
    ...
    qmp_migrate(arg.uri, arg.has_channels, arg.channels, arg.has_detach, arg.detach, arg.has_resume, arg.resume, &err);
    if (err) {
...
void qmp_marshal_query_migrationthreads(QDict *args, QObject **ret, Error **errp)
{
    ...
    ov = qobject_output_visitor_new_qmp(ret);
    if (visit_type_MigrationThreadInfoList(ov, "unused", &retval, errp)) {
        visit_complete(ov, ret);
    }
    visit_free(ov);
    ov = qapi_dealloc_visitor_new();
    visit_type_MigrationThreadInfoList(ov, "unused", &retval, NULL);
    visit_free(ov);
